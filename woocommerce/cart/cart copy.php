			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						// Lógica para imagem padrão baseada na categoria
						$placeholder_image = get_site_url() . '/wp-content/themes/storebiz/assets/images/Acessórios.png';
						if (has_term('desktop', 'product_cat', $product_id)) {
							$placeholder_image = get_site_url() . '/wp-content/themes/storebiz/assets/images/Desktop.png';
						}
						if (has_term('notebook', 'product_cat', $product_id)) {
							$placeholder_image = get_site_url() . '/wp-content/themes/storebiz/assets/images/Desktop.png';
						}
						if (has_term('Acessórios', 'product_cat', $product_id)) {
							$placeholder_image = get_site_url() . '/wp-content/themes/storebiz/assets/images/Acessórios.png';
						}
						if (has_term('Serviço', 'product_cat', $product_id)) {
							$placeholder_image = get_site_url() . '/wp-content/themes/storebiz/assets/images/Serviço.png';
						}
						
						// Verificar se o produto tem imagem, se não tiver usar a padrão
						$product_image = $_product->get_image();
						if (empty($product_image) || strpos($product_image, 'placeholder') !== false) {
							$product_image = '<img src="' . esc_url($placeholder_image) . '" alt="' . esc_attr($product_name) . '" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail" />';
						}
